export default async function getWatchlist() {
    ...
    parsedPinglist.forEach((user) => {
        user.stockSymbols.forEach((stock) => { // Iterate through stockSymbols array of each user.
            if (!stockSymbolArray.includes(stock.symbol)) { // Check if the symbol is already in the array.
