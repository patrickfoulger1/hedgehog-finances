...
export const checkUserWatchlist = async (
  userId: string,
  stockSymbol: string
) => {
...
export async function updateSubscriberToken(
  ...
  await novu.subscribers.credentials.update(
    { providerId: "fcm", credentials: { deviceTokens: [token] } },
    subscriberId
  );
}

export async function appendSubscriberToken(
  token: string,
  subscriberId: string
) {
...
export const updateContactPrefs = async (
  userId: string,
  stockSymbol: string,
  channel: string,
  state: boolean
