...
const removeContactPrefs = async (userId: string, stockSymbol: string) => {
    try {
        await prisma.contactPreferences.deleteMany({
            where: {
                userId,
                stockSymbol,
            },
        ...
        Promise.reject(() => {
            error: "couldn't remove contact prefs";
        });
    }
};
...
export const updateWatchlist = async (userId: string, stockSymbol: string, buttonState: boolean) => {
    if (buttonState) {
        ...
        await removeContactPrefs(userId, stockSymbol);
    } else {
        try {
...
export const triggerWorkflow = async (subscriberId: string, title: string, body: string, stock: string, workflowId: string, prefs: any) => {
    await novu
        .trigger({ workflowId: workflowId, payload: { title: title, body: body, stock: stock, prefs }, to: { subscriberId: subscriberId } })
        .then((results) => {
